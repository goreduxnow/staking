...
// File contracts/masterStake.sol

pragma solidity ^0.8.4;

contract ReduxStake is Ownable{
    ...
    //function to update Block Reward
    function updateBlockRewards(uint256 _rewardPerBlock)public onlyOwner{
        rewardPerBlock = _rewardPerBlock;
    }
    ...
    function getRewardMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {        
        if (_from >= rewardEndBlock) {
            // No extra reward is required.
            return _to.sub(_from);
        } else if (_to <= rewardEndBlock) {            
            return _to.sub(_from).mul(BONUS_MULTIPLIER);
        } else {
            // Some are extra reward blocks and some are not.
            return
                rewardEndBlock.sub(_from).mul(BONUS_MULTIPLIER).add(
                    _to.sub(rewardEndBlock)
                );
        }
    }
    ...
    function add(
        ...
        poolInfo.minAmount = _minAmount;
        poolInfo.lastRewardBlock = lastRewardBlock;
        poolInfo.accRewardPerShare = 0;
    }

    // Update the given pool's min amount. Can only be called by the owner.
    function set(
        uint256 _minAmount,
        bool _withUpdate
    ) public onlyOwner {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getRewardMultiplier(pool.lastRewardBlock, block.number);
            uint256 Reward =
                multiplier.mul(rewardPerBlock);
    ...
    function deposit(uint256 _amount) public {
     ...
     function withdraw(uint256 _amount) public {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "withdraw: amount is not enough");
        updatePool();
        uint256 pending =
        ...
        if(block.timestamp < lockingPeriod){
            // In case of withdrawing token before the locking period.
            user.amount = user.amount.sub(_amount).sub(user.amount.mul(earlyWithdrawTax).div(100));
            pool.lpToken.safeTransfer(address(msg.sender), _amount.sub(user.amount.mul(earlyWithdrawTax).div(100)));
        }
        else{
